...
tz_shp@data$clust_HHnc <-tz_shp@data$go_HH
tz_shp@data$clust_LLnc <-tz_shp@data$go_LL
tz_shp@data$clust_LHnc <-tz_shp@data$go_LH
tz_shp@data$clust_HLnc <-tz_shp@data$go_HL
...
getCluster<-function(indat){
  indat$cluster<-0
  indat$cluster[indat$clust_HHnc==1]<-1
  indat$cluster[indat$clust_LLnc==1]<-2
  indat$cluster[indat$clust_LHnc==1]<-3
  indat$cluster[indat$clust_HLnc==1]<-4
...
p4<-ggplot(go_small, aes(x = POPMEAN, y = GOMEAN2, color=cluster.f)) + 
  geom_point() + 
