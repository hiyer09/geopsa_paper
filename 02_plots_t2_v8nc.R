...
tz_shp@data$clust_HH_2 <-tz_shp@data$t2_HH
tz_shp@data$clust_LL_2 <-tz_shp@data$t2_LL
tz_shp@data$clust_LH_2 <-tz_shp@data$t2_LH
tz_shp@data$clust_HL_2 <-tz_shp@data$t2_HL
...
getCluster<-function(indat){
  indat$cluster<-0
  indat$cluster[indat$t2_HH==1]<-1
  indat$cluster[indat$t2_LL==1]<-2
  indat$cluster[indat$t2_LH==1]<-3
  indat$cluster[indat$t2_HL==1]<-4
...
p2<-ggplot(go_small, aes(x = POPMEAN, y = T2MEAN, color=cluster.f)) + 
  geom_point() + 
